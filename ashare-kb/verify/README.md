...
没有"我读过原文了"。这是刻意的：人工确认是这套系统最明显的后门，
一旦留着，三个月后库里就会堆满谁也说不清凭什么的 verified 条目。

**这道门只挡形式，不挡内容。** `print("HOLDS: ok")` 也是一个脚本，
跑完照样 verified —— 那就是"我读过了"换了身衣服。
**脚本必须真的打开文件、真的做比较。** 做不到就 `kv.inconclusive()`。
...
| 其他 / 缺结论行 | — | **不确定**：`lead` 不变；**`verified` 降为 `stale`** |
...
但"不确定"也不能让一条**已经是 verified** 的断言原地不动 —— 那是同一个
错误的另一个方向：库继续声称一件当前证据支持不了的事。所以 verified 重验
回来不确定时降为 `stale`（需要重新确认），而不是留着或直接 falsified。

这正是这个库要抓的场景：法规修订 → 重新取快照 → 短语不在了 → 脚本说不确定。
...
`kv.holds()` 会在结论行之前打印 `EVIDENCE: <快照名> <sha256>`，
`kb verify` 把这些指纹存进库。于是"这条断言是对着哪份证据验过的"
变成可复核的事实，而不是记忆 —— `kb doctor` 之后就能发现
"验的时候是这份原文，现在不是了"。用 `require_phrases` / `require_across`
的话这是自动的。

快照短于 200 字会直接判"没抽对"（扫描版 PDF？抓到的是导航页？），
而不是报"短语找不到" —— 法规原文不会只有两百字，把取数的问题
说成短语的问题会把人往错方向支。
