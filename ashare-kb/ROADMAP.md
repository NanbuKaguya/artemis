...
`CLAUDE.md` 的硬闸：`verified` 还是 0 的时候不准改代码。排查照做，
**发现记在这里，不要修**。

一个从来没产出过任何东西的系统里的 bug，影响是零 —— 没有 verified 断言
会被它污染，因为一条都没有。修它不是紧急，是舒适。

攒着也不会烂，而且攒着的时候你看得见它们的数量，比修掉之后什么都看不见
要诚实。闸开之后按严重性排序再处理。

<!-- 格式：- [ ] <症状> — <复现> — <发现日期> -->

- [ ] `kb_cli/commands.py` 的报错文案在说假话 —— 「人工「我读过了」是这套
      系统唯一的后门，**已焊死**」。它没焊死：`print("HOLDS: ok")` 也是一个
      脚本，跑完照样 verified。文案该改成"脚本必须真的核对证据"。
      — 复现：给任意 lead 写一个只有 print 的脚本，`kb verify` 会给它 verified
      — 2026-09-19
      — 注：这是 `.py` 文件里的字符串，按硬闸不碰。两处 `.md` 里的同一句
        已经改了（那是文档）。第一次试探就给自己开"字符串不算代码"的口子，
        闸就白立了。
